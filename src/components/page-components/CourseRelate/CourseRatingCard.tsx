...
export default function CourseRatingCard({
  ...
  // ✅ Guard against NaN, negative, or >5
  const safeAverage =
    typeof averageRating === "number" && isFinite(averageRating)
      ? Math.min(Math.max(averageRating, 0), 5)
      : 0;

  const filledStars = Math.floor(safeAverage);
  const hasHalfStar = safeAverage % 1 >= 0.5;
  const emptyStars = Math.max(5 - filledStars - (hasHalfStar ? 1 : 0), 0);
  ...
  return (
    <div className="flex gap-2 border border-gray-300 rounded-lg p-4 mt-6">
      ...
      <div className="ml-auto flex flex-col justify-center items-end">
        ...
        <h1 className="text-sm text-gray-500">{totalReviews ?? 0} Reviews</h1>
      </div>
    </div>
  );
}
